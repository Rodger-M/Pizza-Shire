...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["usuario"]))){
        ...
        $sql = "SELECT cod_adm FROM Administrador WHERE usuario = ?";
    ...
    if(empty($nome_erro) && empty($sobrenome_erro) && empty($cpf_erro) && empty($email_erro) && empty($usuario_erro) && empty($senha_erro) && empty($conf_senha_erro)){
        ...
        $sql = "INSERT INTO Administrador (nome, sobrenome, cpf, email, usuario, senha) VALUES (?, ?, ?, ?, ?, ?)";
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>Already have an account? <a href="loginadm.php">Login here</a>.</p>
        </form>
    </div>    
</body>
</html>